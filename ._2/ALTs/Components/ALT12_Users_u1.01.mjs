...
##FNCS     .--------------------+----------------------------------------------+
#                               |
# async func getUsers            | ALT11  get keys {aPlatform}                                              // .(50310.02.9) 
# async func putKeys            | ALT11  put key  {App}                                                     // .(50310.03.9) 
#                               |
##CHGS     .--------------------+----------------------------------------------+
#.(50309.04   3/09/25 RAM  7:00p| Create ALT12_Users_Component.mjs
...
async  function  getKeys(   aPlatforms, aModel  ) {                                                         // .(50310.02.10 write getKeys Beg) 
       ...
       var  mKeys            =  await pResponse.json();
        if (aPlatforms > '') {    
            ...
            FRT.sayMsg( `ALT11[  80]  aPlatforms: '${aPlatforms}'`, -1 )
...
//          console.error( '    Error fetching keys:', pError.message );
            ...
            FRT.sayMsg( `ALT11[  92]  Error: Getting Key for ${aPlatforms}, aModel: '${aModel}'`, -1 )
            }
...
//     var  aEnv_path         =  FRT.path( aApp_path,     'utils/FRTs/_env' )
       ...
       var  aEnv_path         =  FRT.path( FRT.__basedir, `../${pEnv.Repo}`, pEnv.AppPath, pEnv.File )  
            FRT.sayMsg( `ALT11[  89]  aEnv_path: '${aEnv_path}'`, -1)
       var  aEnv_file         =  FRT.readFileSync(  aEnv_path )  
            aEnv_file         =  aEnv_file.replace(/  ANYLLM_API_KEY +=.+/, `  ANYLLM_API_KEY = "${aKey}"` ) 
                                 FRT.writeFileSync( aEnv_path, aEnv_file )                                  
            FRT.sayMsg( `ALT11[  93]  Saved Key for App: '${aApp}'`, -1)
    return  aKey 
        } catch (pError) {
            FRT.sayMsg( `ALT11[  96]  Error: Putting Key for app: ${aApp}`, -1 )
    return  ''
            }
